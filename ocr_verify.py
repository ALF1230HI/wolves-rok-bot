...
resource(s) and amount(s) the member typed, somewhere in its transport
history. This does NOT check who the transport was sent to — officers
manually confirm the recipient (the alliance bank) themselves.
"""
...
# Kept only for the legacy screenshot_shows_bank_donation() helper below.
# The main /donate flow no longer checks the transport target at all.
...
def _verify_sync(image_bytes: bytes, expected_resources: dict) -> tuple[bool, str]:
    ...
    history (not just the newest entry), a row for each resource/amount in
    `expected_resources` (e.g. {"Food": 5000000}) — regardless of who the
    transport target was. (Officers manually verify the recipient
    themselves; this only checks the resource type + amount typed by the
    member actually appears in their screenshot.)

    Each report row only shows one resource type, so for every resource the
    member typed we look for any row of that resource type whose amount
    matches (within tolerance) — it doesn't have to be the most recent entry,
    and it doesn't have to be targeted at the bank. Returns (ok, reason);
    ...
    readable_rows = [r for r in rows if r["resource"] is not None and r["amount"] is not None]
    if not readable_rows:
        return False, (
            "Found entries in your screenshot but couldn't read their resource/amount clearly. "
            "Try a clearer, uncropped screenshot."
        )

    # For each resource the member typed, look for ANY row of that resource
    # type with a matching amount, anywhere in the screenshot.
    mismatches = []
    for resource, expected_amount in expected_resources.items():
        if not expected_amount:
            continue
        candidates = [r["amount"] for r in readable_rows if r["resource"] == resource]
        if not candidates:
            mismatches.append(
                f"you entered {expected_amount:,} {resource}, but the screenshot doesn't show any {resource} transports"
            )
            continue
        match_found = any(
        ...
        if not match_found:
            ...
            mismatches.append(
                f"you entered {expected_amount:,} {resource}, but the screenshot shows {shown} {resource} instead"
            )
